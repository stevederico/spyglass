...
/** An App Store Connect app summary. */
interface AscApp {
  id: string;
  name: string;
  bundleId?: string;
...
/** Aggregated analytics metrics for an app. */
...
export default function AnalyticsView() {
  ...
  const upgradeSheetRef = useRef<UpgradeSheetHandle>(null);
  ...
  useEffect(() => {
    ...
    const loadApps = async () => {
      try {
        const data = await apiRequest<AscApp[]>('/asc/apps');
   ...
   * @param app - App object with id and name
   */
  const handleSelectApp = async (app: AscApp) => {
    const usage = await getRemainingUsage('analytics');
    ...
    setIsLoading(true);
    try {
      const data = await apiRequest<AnalyticsMetrics>(`/asc/analytics/metrics?appId=${app.id}`);
      setMetrics(data);
