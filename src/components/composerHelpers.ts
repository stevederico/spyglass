...
/** Dimension specification for a single App Store Connect device size. */
...
/** Layer visibility flags controlling which composite layers render. */
...
/** Frame model dimensions used when overlaying a device frame PNG. */
export interface FrameModelInfo {
  frameWidth: number;
  frameHeight: number;
  screenWidth: number;
  screenHeight: number;
  /** ASC screenshot tier this frame maps to (used by export logic). */
  ascTier?: string;
}

/** Full composer state describing one screenshot composition. */
export interface ComposerState {
  ...
  /** Frame model slug (present on slot-derived state). */
  frameModel?: string;
  /** Selected custom font family (present on slot-derived state). */
  selectedFont?: string;
}
 ...
 */
export function getDeviceFamily(deviceKey: string): DeviceFamily {
 ...
 */
export function detectFamilyFromImage(img: HTMLImageElement): DeviceFamily {
 ...
 */
export function getDefaultDeviceForFamily(family: DeviceFamily): string {
 ...
 */
export function buildFontString(fontWeight: string, fontSize: number, fontFamily?: string): string {
  return fontFamily
 ...
 */
/** Background drawing settings derived from the composer state. */
...
function drawBackground(ctx: CanvasRenderingContext2D, w: number, h: number, settings: BackgroundSettings): void {
  ...
  if (isGradient) {
    ...
    grad.addColorStop(0, gradientStart ?? bgColor);
    grad.addColorStop(1, gradientEnd ?? bgColor);
    ctx.fillStyle = grad;
  } else {
 ...
 */
function drawImageCover(ctx: CanvasRenderingContext2D, img: HTMLImageElement, dx: number, dy: number, dw: number, dh: number): void {
...
function drawDeviceFrame(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, radius: number, bezelWidth: number): void {
 ...
 */
function drawMarketingText(ctx: CanvasRenderingContext2D, text: string | undefined, x: number, y: number, maxWidth: number, fontSize: number, color: string, shadowBlur: number | boolean | undefined, fontWeight: string, maxHeight?: number, fontFamily?: string): void {
  if (!text) return;

  ctx.fillStyle = color;
  ctx.textAlign = 'center';

  // Boolean `true` (from a toggle) coerces to 1 here, matching prior JS behavior.
  const blur = Number(shadowBlur);
  if (blur > 0) {
    ctx.shadowColor = 'rgba(0, 0, 0, 0.5)';
    ctx.shadowBlur = blur;
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = Math.round(blur / 2);
  }
 ...
 */
export function drawComposite(canvas: HTMLCanvasElement, state: ComposerState): void {
  ...
  if (vis.device && isFullscreen) {
    ...
    if (hasFramePNG && state.frameModelInfo && state.frameImage) {
...
/** A named preset of composer settings applied as a starting point. */
export interface StarterTemplate {
  name: string;
  settings: Partial<ComposerState>;
}
 ...
 */
export function renderForLocale(referenceCanvas: HTMLCanvasElement, baseState: ComposerState, localeCode: string, line1: string, line2: string): Promise<Blob> {
 ...
 */
export function exportCanvasPNG(canvas: HTMLCanvasElement, deviceKey: string): void {
  canvas.toBlob((blob) => {
    if (!blob) {
