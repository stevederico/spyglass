...
/** A single metadata precheck warning returned by the backend. */
...
export default function PrecheckView() {
  ...
  const [warnings, setWarnings] = useState<PrecheckWarning[] | null>(null);
  ...
  const handlePrecheck = async () => {
    ...
    const usage = await getRemainingUsage('precheck');
    ...
    setIsLoading(true);
    try {
      const data = await apiRequest<{ warnings: PrecheckWarning[] }>('/precheck', {
        method: 'POST',
        body: JSON.stringify({ name, description, keywords, promotionalText })
      });
      setWarnings(data.warnings);
      await trackUsage('precheck');
    } catch {
