...
import { useSlotHistory } from './useSlotHistory';
...
import { DEVICES, FONT_WEIGHTS, drawComposite, exportCanvasPNG, renderForLocale, getDeviceFamily, detectFamilyFromImage, getDefaultDeviceForFamily } from './composerHelpers';
import type { LayerVisibility, ComposerState } from './composerHelpers';
import type { SlotTranslation } from './useSlots';
import { FRAME_MODELS } from './frameManifest';
import { loadFrame, preloadFrame } from './frameLoader';
...
/** Badge style variants supported by the shadcn Badge component. */
type BadgeVariant = 'default' | 'secondary' | 'destructive' | 'outline' | 'ghost' | 'link';

/** Remaining translation character quota for the account. */
interface TranslationQuota {
 ...
 * @returns {{ label: string, variant: string }} Badge display properties
 */
function getStatusBadge(status: string | undefined): { label: string; variant: BadgeVariant } {
  switch (status) {
    case 'original':
...
export default function ScreenshotsView() {
  ...
  const [translationQuota, setTranslationQuota] = useState<TranslationQuota | null>(null);
  const [showTranslations, setShowTranslations] = useState(false);
  const [selectedLocales, setSelectedLocales] = useState<Set<string>>(() => new Set(LOCALES.filter((l) => !ENGLISH_LOCALES.has(l.code)).map((l) => l.code)));

  /**
   * Toggle visibility of a named layer
   *
   * @param name - Layer key to toggle
   */
  function toggleLayer(name: keyof LayerVisibility) {
  ...
  useEffect(() => {
    apiRequest<TranslationQuota>('/translate/quota').then(setTranslationQuota).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    if (historyState.bgColor !== undefined) setBgColor(historyState.bgColor);
    if (historyState.isGradient !== undefined) setIsGradient(historyState.isGradient);
    if (historyState.gradientStart !== undefined) setGradientStart(historyState.gradientStart);
    if (historyState.gradientEnd !== undefined) setGradientEnd(historyState.gradientEnd);
    if (historyState.gradientDirection !== undefined) setGradientDirection(historyState.gradientDirection);
    if (historyState.textLine1 !== undefined) setTextLine1(historyState.textLine1);
    if (historyState.textLine2 !== undefined) setTextLine2(historyState.textLine2);
    if (historyState.textPosition !== undefined) setTextPosition(historyState.textPosition);
    if (historyState.fontSize !== undefined) setFontSize(historyState.fontSize);
    if (historyState.textColor !== undefined) setTextColor(historyState.textColor);
    if (historyState.textShadow !== undefined) setTextShadow(historyState.textShadow);
    if (historyState.fontWeight !== undefined) setFontWeight(historyState.fontWeight);
    if (historyState.autoFitText !== undefined) setAutoFitText(historyState.autoFitText);
    if (historyState.device !== undefined) setDevice(historyState.device);
    if (historyState.showBezel !== undefined) setShowBezel(historyState.showBezel);
  ...
  async function handleGenerateBg() {
    ...
    try {
      const response = await apiRequest<{ image?: string; error?: string }>('/ai/generate-background', {
        method: 'POST',
   ...
   *
   * @param img - Loaded screenshot image
   */
  function autoSwitchDevice(img: HTMLImageElement) {
   ...
   *
   * @param e - Click event on the canvas element
   */
   ...
   *
   * @param lineNum - Which text line (1 or 2)
   * @returns CSS style for the overlay input
   */
  function getTextOverlayStyle(lineNum: number): CSSProperties {
    const canvas = canvasRef.current;
    if (!canvas) return {};
   ...
   *
   * @param settings - Template settings object
   */
  const handleLoadTemplate = useCallback((settings: Partial<ComposerState>) => {
    if (!settings) return;
   ...
   *
   * @param code - Locale code to toggle
   */
  ...
  const handleTranslateSelected = useCallback(async () => {
    ...
    try {
      ...
      for (const locale of toTranslate) {
        newTranslations[locale.code] = { line1: '', line2: '', status: 'error', errorMessage: message };
      }
    }
   ...
   *
   * @param localeCode - Locale code being edited
   * @param field - Which line is being edited
   * @param value - New translation text
   */
  const handleCellEdit = useCallback((localeCode: string, field: 'line1' | 'line2', value: string) => {
  ...
  const handleCopyAll = useCallback(async () => {
    ...
    const output: Record<string, { name: string; line1?: string; line2?: string }> = {};
   ...
   *
   * @param modelKey - Frame model key from FRAME_MODELS, or '' to clear
   */
  function handleFrameModelChange(modelKey: string) {
   ...
   *
   * @param deviceKey - Device key from DEVICES
   */
    ...
    const draw = () => {
      try {
        drawComposite(canvas, {
          ...
          bgColor, isGradient, gradientStart, gradientEnd, gradientDirection, bgImage,
          autoFitText, fontFamily: selectedFont, editingLine: editingLine ?? undefined, layers,
          frameImage, frameModelInfo: frameModel ? FRAME_MODELS[frameModel] : null,
  ...
  return (
    <>
      <Header title="" className="[&>div>div:last-child]:ml-0 [&>div>div:last-child]:flex-1">
        <AppPicker />
        {selectedApp && (
          <Select value={previewLocale === 'original' ? 'en-US' : previewLocale} onValueChange={(v) => v && setPreviewLocale(v)}>
        ...
        <Select value={device} onValueChange={(v) => v && handleDeviceChange(v)}>
                    ...
                    {isGradient && (
                      <div className="flex flex-col gap-1.5 border-l-2 border-border/30 pl-2.5">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <Select value={gradientDirection} onValueChange={(v) => v && setGradientDirection(v)}>
            ...
            <div className="border-b border-border/40 px-3 py-2.5">
              ...
              <div id="section-text" className={`flex flex-col gap-2 ${collapsedSections.text ? 'hidden' : ''}`}>
                <div className="flex items-center justify-between">
                  ...
                  <Select value={textPosition} onValueChange={(v) => v && setTextPosition(v)}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Select value={selectedFont || 'system-default'} onValueChange={(val) => setSelectedFont(val === 'system-default' || !val ? '' : val)}>
            ...
            <div className="border-b border-border/40 px-3 py-2.5">
              ...
              <div id="section-device" className={`flex flex-col gap-2 ${collapsedSections.device ? 'hidden' : ''}`}>
                <div className="flex items-center justify-between">
                  ...
                  <Select value={frameModel} onValueChange={(val) => handleFrameModelChange(val === 'none' || !val ? '' : val)}>
                ...
                {translationQuota && (
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] text-muted-foreground">
                      {(translationQuota.charsRemaining ?? 0).toLocaleString()} / {(translationQuota.limit ?? 0).toLocaleString()} chars remaining
                    </p>
