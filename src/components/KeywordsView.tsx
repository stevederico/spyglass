...
/** A single App Store search result for a keyword. */
...
export default function KeywordsView() {
  ...
  const handleSearch = async () => {
    ...
    const usage = await getRemainingUsage('keyword-search');
    ...
    setIsLoading(true);
    try {
      const data = await apiRequest<KeywordResult[]>(`/keywords/search?term=${encodeURIComponent(searchTerm)}`);
      setResults(data);
      await trackUsage('keyword-search');
    } catch {
