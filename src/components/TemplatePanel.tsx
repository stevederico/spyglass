...
/** A saved screenshot template returned by the templates API. */
interface Template {
  id: string;
  name: string;
...
/** A custom uploaded font returned by the fonts API. */
interface Font {
  id: string;
  name: string;
}
...
interface TemplatePanelProps {
  /** Current screenshot state to save as a template */
  currentState: Partial<ComposerState>;
  /** Callback to apply a template's settings */
  onLoadTemplate: (settings: Partial<ComposerState>) => void;
  /** Current app ID used to scope templates */
  appId: string;
  /** Currently selected custom font family name */
  selectedFont: string;
  /** Callback when the font selection changes */
  onFontChange: (font: string) => void;
}

export default function TemplatePanel({ currentState, onLoadTemplate, appId, selectedFont, onFontChange }: TemplatePanelProps) {
  ...
  const fetchTemplates = useCallback(async (): Promise<Template[]> => {
    if (!appId) return [];
    try {
      const data = await apiRequest<Template[]>(`/templates?appId=${encodeURIComponent(appId)}`);
  ...
  const fetchFonts = useCallback(async () => {
    try {
      const data = await apiRequest<Font[]>('/templates/fonts');
   ...
   *
   * @param id - App ID to seed templates for
   */
  const seedStarterTemplates = useCallback(async (id: string) => {
   ...
   */
  const handleFontUpload = useCallback(async (e: ChangeEvent<HTMLInputElement>) => {
