...
/** An export package record returned by the exports API. */
...
/** A single file within an export package. */
 ...
 *
 * @param status - Package status (ready, uploaded, draft)
 * @returns Badge display properties
 */
...
export default function ExportsView() {
  ...
  const [expandedFiles, setExpandedFiles] = useState<ExportFile[] | null>(null);
   ...
   *
   * @param appId - App identifier to filter packages by
   */
  const fetchPackages = useCallback(async (appId: string) => {
    setIsLoading(true);
    try {
      const result = await apiRequest<ExportPackage[]>(`/exports?appId=${encodeURIComponent(appId)}`);
      setPackages(result || []);
    } catch (err) {
   ...
   */
  const fetchPackageFiles = useCallback(async (id: string) => {
    try {
      const result = await apiRequest<{ files?: ExportFile[] }>(`/exports/${id}`);
   ...
   *
   * @param pkg - Package object with id and app_name
   */
  const handleDownload = useCallback(async (pkg: ExportPackage) => {
   ...
   *
   * @param id - Package identifier to delete
   */
   ...
   *
   * @param id - Package identifier to toggle
   */
      ...
      <Dialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <DialogContent aria-describedby={undefined}>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={() => deleteId && handleDelete(deleteId)} aria-label="Confirm delete">Delete</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
