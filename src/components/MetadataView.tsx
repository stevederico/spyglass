...
/** Localized App Store listing fields for a single locale. */
interface Metadata {
  name: string;
  subtitle: string;
  description: string;
...
/** A keyword suggestion returned by the ASO suggestion endpoint. */
interface KeywordSuggestion {
  keyword: string;
  /** Relative search-volume label, e.g. "high", "med", "low". */
  volume?: string;
}

/** A saved version-history snapshot of metadata. */
 ...
 *
 * @component
 * @returns Styled character count display
 */
 ...
 *
 * @component
 * @returns Diff display or null if unchanged
 */
function DiffField({ original, current, label }: { original: string; current: string; label: string }) {
...
export default function MetadataView() {
  ...
  const [localizedMetadata, setLocalizedMetadata] = useState<MetadataMap>({});
  const [originalMetadata, setOriginalMetadata] = useState<MetadataMap>({});
  const [localizationIds, setLocalizationIds] = useState<Record<string, string>>({});
   ...
   *
   * @param appId - App Store Connect app identifier
   */
  async function fetchMetadata(appId: string) {
   ...
   *
   * @param field - Field name in metadata state
   * @param value - New field value
   */
  function handleFieldChange(field: keyof Metadata, value: string) {
   ...
   */
  async function handleGenerateField(field: GeneratableField) {
    ...
    try {
      const endpoints: Record<GeneratableField, string> = {
        description: '/ai/generate-description',
        keywords: '/ai/generate-keywords',
        whatsNew: '/ai/generate-whats-new'
      };
      const bodies: Record<GeneratableField, Record<string, unknown>> = {
      ...
      const result = await apiRequest<Record<string, string>>(endpoints[field], {
        method: 'POST',
   ...
   *
   * @param field - Field to improve
   */
  async function handleImproveField(field: 'description' | 'whatsNew') {
    ...
    try {
      const result = await apiRequest<{ improved?: string }>('/ai/improve-text', {
        method: 'POST',
        body: JSON.stringify({
          text,
          field,
   ...
   *
   * @param field - Metadata field to translate
   */
  async function handleTranslateField(field: keyof Metadata) {
    ...
    for (const locale of nonEnglish) {
      try {
        const response = await apiRequest<{ translations?: string[] }>('/translate/batch', {
          method: 'POST',
          body: JSON.stringify({
            texts: [sourceText],
            source: 'en',
  ...
  async function handleSuggestKeywords() {
    ...
    try {
      const result = await apiRequest<{ suggestions?: KeywordSuggestion[] }>('/ai/suggest-keywords', {
        method: 'POST',
        body: JSON.stringify({
          appName,
   ...
   */
  function handleAddSuggestion(keyword: string) {
   ...
   *
   * @param snapshotId - Snapshot identifier to restore
   */
  ...
  const diffFields: { key: keyof Metadata; label: string }[] = [
