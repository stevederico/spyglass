...
/**
 * Validate parsed config.json matches the expected shape.
 *
 * Narrows unknown JSON before it is trusted as the backend config. Checks the
 * fields the loader reads: optional `staticDir` (string) and a `database` block
 * with `db`, `dbType`, and `connectionString` (all strings).
 *
